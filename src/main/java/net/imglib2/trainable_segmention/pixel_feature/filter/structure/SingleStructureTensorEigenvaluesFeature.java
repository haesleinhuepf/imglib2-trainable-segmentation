...
public class SingleStructureTensorEigenvaluesFeature extends AbstractFeatureOp {
	...
	public void apply(CLIJFeatureInput input, List<CLIJView> output) {
		...
		try (
				...
				CLIJMultiChannelImage blurredProducts = blur(input.gpuApi(), products.channels(), integrationGauss, input.targetInterval());
		) {
	...
	private CLIJMultiChannelImage blur(GpuApi gpu, List<CLIJView> products, NeighborhoodOperation integrationGauss, Interval targertInteval) {
