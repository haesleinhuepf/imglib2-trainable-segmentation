...
public class SingleStructureTensorEigenvaluesFeature extends AbstractFeatureOp {
		...
		RandomAccessibleInterval<DoubleType> derivatives)
	{
		...
		RandomAccessibleInterval<DoubleType> output = RevampUtils.createImage(outputInterval, new DoubleType());
		...
		Interval derivativeInterval)
	{
		...
		RandomAccessibleInterval<DoubleType> tmp = RevampUtils.createImage(RevampUtils.appendDimensionToInterval(
	...
	public void apply(CLIJFeatureInput input, List<CLIJView> output) {
		...
		try (
				...
				CLIJMultiChannelImage blurredProducts = blur(input.gpuApi(), products.channels(), integrationSigma, border);
		) {
	...
	private CLIJMultiChannelImage blur(GpuApi gpu, List<CLIJView> products, double[] integrationSigma, long[] border) {
		...
		try (
			...
			CLIJMultiChannelImage blurred = new CLIJMultiChannelImage(gpu, outputDimensions, products.size());
			...
			for (int i = 0; i < products.size(); i++) {
				...
				if(integrationSigma.length == 3)
					gpu.gaussianBlur(tmpInput, tmpOutput, integrationSigma[0], integrationSigma[1], integrationSigma[2]);
				else
