...
public class SingleGaborFeature extends AbstractFeatureOp {
	...
	private List<RandomAccessibleInterval<FloatType>> initGaborKernels(double sigma, double gamma, double psi,
		double frequency, int nAngles)
	{
		...
		for (int i = 0; i < nAngles; i++) {
			...
			RandomAccessibleInterval<FloatType> kernel = RevampUtils.createImage(interval, new FloatType());
	...
	private static void garborKernel(RandomAccessibleInterval<FloatType> kernel, double psi, double frequency,
		...
		RandomAccessibleInterval<FloatType> min)
	{
		...
		RandomAccessibleInterval<FloatType> stack = RevampUtils.createImage(RevampUtils.appendDimensionToInterval(interval, 0,
