...
public class SingleStatisticsFeature extends AbstractFeatureOp {
	...
	private void calculateMeanAndVariance(GpuApi gpu, int[] windowSize, GpuView original, Iterator<GpuView> iterator) {
		if(!mean && !variance)
			return;
		if (mean) {
			...
			try( GpuImage mean = gpu.create(Intervals.dimensionsAsLongArray(variance.dimensions()), NativeTypeEnum.Float) ) {
	...
	private void calculateVariance(GpuApi gpu, int[] windowSize, GpuView original, GpuView mean, GpuView variance) {
		try (
				GpuImage squared = gpu.create(Intervals.dimensionsAsLongArray(original.dimensions()), NativeTypeEnum.Float);
				GpuImage meanOfSquared = gpu.create(Intervals.dimensionsAsLongArray(variance.dimensions()), NativeTypeEnum.Float);
		) {
			long n = Intervals.numElements(windowSize);
			if(n <= 1)
