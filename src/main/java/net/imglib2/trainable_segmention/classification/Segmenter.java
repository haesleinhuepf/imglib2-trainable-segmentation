...
public class Segmenter {
	...
	private void segmentCpu(RandomAccessible<?> image, RandomAccessibleInterval<? extends IntegerType<?>> out) {
		...
		LoopBuilder.setImages(Views.collapseReal(featureValues), out).multiThreaded().forEachChunk( chunk -> {
		...
		RandomAccessibleInterval<?> image)
	{
		...
		RandomAccessibleInterval<FloatType> result = RevampUtils.createImage(RevampUtils.appendDimensionToInterval(
	...
	private void predictCpu(RandomAccessibleInterval<? extends RealType<?>> out, RandomAccessible<?> image) {
		...
		LoopBuilder.setImages(Views.collapseReal(featureValues), Views.collapse(out)).multiThreaded().forEachChunk(
				chunk -> {
					CompositeInstance compositeInstance = new CompositeInstance(null, attributesAsArray());
					chunk.forEachPixel((input, output) -> {
						compositeInstance.setSource(input);
	...
	private void predictGpu(RandomAccessibleInterval<? extends RealType<?>> out, RandomAccessible<?> image) {
		...
		try(
				...
				CLIJMultiChannelImage distribution = new CLIJMultiChannelImage(clij, featureStack.getSpatialDimensions(), features.count())
		)
		{
