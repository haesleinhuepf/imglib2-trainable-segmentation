...
class OpenCLSyntax {
	...
	private static Set<String> initSetOfReservedWords() {
		List<String> scalarTypes = Arrays.asList( "char", "uchar", "short", "ushort", "int", "uint", "long", "ulong",
				"float", "double");
		List<String> vectorTypes = anyCombination(scalarTypes, Arrays.asList("2", "4", "8", "16", "32"));
		HashSet<String> result = new HashSet<>();
		result.addAll(scalarTypes);
		result.addAll(vectorTypes);
		result.addAll(Arrays.asList("void", "unsigned", "signed", "local", "global", "constant", "private", "__local",
				"__global", "__constant", "__private", "image2d_t", "image3d_t", "image2d_array_t", "image1d_t",
				"image1d_buffer_t", "image1d_array_t", "image2d_depth_t", "image2d_array_depth_t", "sampler_t",
