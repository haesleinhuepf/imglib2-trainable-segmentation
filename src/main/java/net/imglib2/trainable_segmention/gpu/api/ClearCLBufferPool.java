 ...
 * <p>
 * The method {@link #create} might be used as usual to get a {@link ClearCLBuffer},
 * but the buffer might has been used before. After use a buffer should be returned
 * by calling {@link #release(ClearCLBuffer)}.
 */
class ClearCLBufferPool implements AutoCloseable {
	...
	public ClearCLBuffer create(long[] dimensions, long numberOfChannels, NativeTypeEnum type) {
		...
		return context.createBuffer(MemAllocMode.Best, HostAccessType.ReadWrite, KernelAccessType.ReadWrite,
	...
	public void release(ClearCLBuffer buffer) {
		...
		Queue<ClearCLBuffer> list = unused.computeIfAbsent(key, ignore -> new ConcurrentLinkedQueue<>());
		list.add(buffer);
	}
