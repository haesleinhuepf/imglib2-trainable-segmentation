...
public class GpuApi implements AutoCloseable {
	...
	public <T extends RealType<?>> RandomAccessibleInterval<T> pullRAIMultiChannel(GpuImage image) {
		return internalPullRai(image, RevampUtils.extend(image.getDimensions(), image.getNumberOfChannels()));
	}
	...
	private <T extends RealType<?>> RandomAccessibleInterval<T> internalPullRai(GpuImage source, long[] dimensions) {
