...
public class CLIJKernelConvolution implements NeighborhoodOperation {
	...
	public static void convolve(GpuApi gpu, GpuImage input, GpuImage kernel, int kernel_center, GpuImage output, int d) {
		HashMap<String, Object> parameters = new HashMap<>();
		...
		defines.put("OUTPUT_WRITE_PIXEL(cx,cy,cz,v)", "WRITE_output_IMAGE(output, POS_output_INSTANCE(" + position(d, 0) + ",0), v)");
