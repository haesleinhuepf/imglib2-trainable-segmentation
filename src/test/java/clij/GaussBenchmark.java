...
public class GaussBenchmark {
	...
	private final CLIJMultiChannelImage derivatives = new CLIJMultiChannelImage(clij, new long[]{98, 98, 98}, 6);
	...
	public RandomAccessibleInterval benchmarkDerivativeCombinedKernel() {
		for (int i = 0; i < 100; i++) {
			...
			CLIJView myInput = CLIJView.interval(input, myOutput.interval());
			List<CLIJView> channels = derivatives.channels();
			CLIJView back0 = CLIJView.interval(myInput.buffer(), Intervals.translate(myInput.interval(), -1, 0));
			CLIJView front0 = CLIJView.interval(myInput.buffer(), Intervals.translate(myInput.interval(), +1, 0));
			CLIJView back1 = CLIJView.interval(myInput.buffer(), Intervals.translate(myInput.interval(), -1, 1));
			CLIJView front1 = CLIJView.interval(myInput.buffer(), Intervals.translate(myInput.interval(), +1, 1));
			CLIJView back2 = CLIJView.interval(myInput.buffer(), Intervals.translate(myInput.interval(), -1, 2));
			CLIJView front2 = CLIJView.interval(myInput.buffer(), Intervals.translate(myInput.interval(), +1, 2));
	...
	private void derivative(CLIJView input, CLIJView output, int d) {
		...
		CLIJView front = CLIJView.interval(input.buffer(), Intervals.translate(input.interval(), +1, d));
