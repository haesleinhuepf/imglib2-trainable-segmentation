...
public class GaussBenchmark {
	...
	private final ClearCLBuffer inputBuffer = gpu.push(RandomImgs.seed(1).nextImage(new FloatType(), interval));
	...
	private final ClearCLBuffer inputBuffer2 = gpu.push(RandomImgs.seed(1).nextImage(new FloatType(), large, large, large));
	private final ClearCLBuffer tmp1 = gpu.create(new long[]{64, large, large}, NativeTypeEnum.Float);
	...
	public RandomAccessibleInterval lowLevelGauss() {
		try(ReuseScope scope = new ReuseScope(buffers))
		{
			...
			ClearCLBuffer output = scope.create(64, 64, 64);
			{
				...
				CLIJKernelConvolution.convolve(gpu, CLIJView.wrap(inputBuffer2), kernel, CLIJView.wrap(tmp1), 0);
			}
			{
				ClearCLBuffer kernel = scope.add(gaussKernel(8));
