...
public class CLIJLoopBuilderTest {
	...
	public void testCLIJViewInput() {
		CLIJView a = CLIJView.interval(gpu.push(ArrayImgs.floats(new float[]{0, 0, 0, 42}, 2, 2)), Intervals.createMinSize(1,1,1,1));
		GpuImage d = gpu.push(ArrayImgs.floats(new float[]{0}, 1, 1));
		CLIJLoopBuilder.gpu(gpu)
				.addInput("a", a)
				.addOutput("d", d)
	...
	public void testCLIJViewOutput() {
		CLIJView a = CLIJView.interval(gpu.create(new long[]{2, 2}, NativeTypeEnum.Float), Intervals.createMinSize(1,1,1,1));
		CLIJLoopBuilder.gpu(gpu)
				.addOutput("a", a)
				.forEachPixel("a = 42");
