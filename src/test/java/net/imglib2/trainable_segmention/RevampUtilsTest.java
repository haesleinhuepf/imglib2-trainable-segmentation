...
public class RevampUtilsTest {
	...
	public void testDeriveX() {
		RandomAccessibleInterval<FloatType> input = Utils.create2dImage(Intervals.createMinMax(-1, -1, 1, 1), (x, y) -> 3 * x + 5 * y);
		RandomAccessibleInterval<FloatType> result = RevampUtils.deriveX(input, new FinalInterval(1, 1));
	...
	public void testDeriveY() {
		RandomAccessibleInterval<FloatType> input = Utils.create2dImage(Intervals.createMinMax(-1, -1, 1, 1), (x, y) -> 3 * x + 5 * y);
		RandomAccessibleInterval<FloatType> result = RevampUtils.deriveY(input, new FinalInterval(1, 1));
	...
	{
		RandomAccessibleInterval<ByteType> inputAccessed = RevampUtils.createImage(inputInterval, new ByteType());
		...
		RandomAccessibleInterval<? extends NumericType<?>> accessed)
	{
		...
		return Converters.convert( accessed, (in, out) -> { in.setOne(); out.setZero(); }, new FloatType());
	}
