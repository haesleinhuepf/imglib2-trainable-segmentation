...
public class GaussBenchmark {
	...
	private final GpuImage inputBuffer = gpu.push(RandomImgs.seed(1).nextImage(new FloatType(), interval));
	...
	private final GpuImage inputBuffer2 = gpu.push(RandomImgs.seed(1).nextImage(new FloatType(), large, large, large));
	...
	public RandomAccessibleInterval lowLevelGauss() {
		try(GpuScope scope = new GpuScope(gpu))
		{
			...
			GpuImage output = scope.create(64, 64, 64);
			{
				...
				CLIJKernelConvolution.convolve(gpu, GpuViews.wrap(inputBuffer2), kernel, GpuViews.wrap(tmp1), 0);
			}
			{
				GpuImage kernel = scope.add(gaussKernel(8));
